...
import dg.hexagonal.domain.logic.FamilyGoOnEventService;
...
public class App {
	...
	public static void main(String[] args) {
		...
		Event olaBirthday = eventService.createEvent("7th Ola's birthday", new DateTime(2019, 1, 5, 10, 30), EventType.FAMILY, monkeyTown);
		Event wojtekBirthday = eventService.createEvent("5th Wojtek's birthday", new DateTime(2019, 1, 15, 12, 30), EventType.FAMILY, ballorig);
		...
		Family familyGrebowiec = familyService.createNewFamily(mother, father, "dominika.grebowiec@gmail.com", kidOla, kidWojtek);
		...
		Family familyKowalscy = familyService.createNewFamily(mother2, father2, "kowalscy@gmail.com", kid21, kid22);
		
		FamilyGoOnEventService familyGoOnEventService = (FamilyGoOnEventService) context.getBean("familyGoOnEventService");
		familyGoOnEventService.familyGoOnEvent(olaBirthday.getEventId(), familyGrebowiec.getFamilyId());
		familyGoOnEventService.familyGoOnEvent(olaBirthday.getEventId(), familyKowalscy.getFamilyId());
		
		familyGoOnEventService.familyGoOnEvent(wojtekBirthday.getEventId(), familyGrebowiec.getFamilyId());
		
		EventParticipants eventParticipants = familyGoOnEventService.getFamiliesGoOn(olaBirthday.getEventId());
		System.out.println("Event participants for event " + olaBirthday.getEventId());
		for (Long id : eventParticipants.getFamiliesGo()) {
